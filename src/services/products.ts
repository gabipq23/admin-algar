...
export class ProductsService {
  async allProductsFiltered({
    ...
    const res = await apiPurchase.get(`/telecom/algar/products`, {
      params: {
        page,
        per_page,
        online,
  ...
  async createProduct(
    ...
    try {
      const response = await apiPurchase.post(`/telecom/algar/products`, data, {
        headers,
      });
  ...
  async getProductById(id: number): Promise<IProduct> {
    const response = await apiPurchase.get(`/telecom/algar/products/${id}`);
  ...
  async uploadProductConditions(id: number, files: File[]): Promise<unknown> {
    ...
    try {
      const response = await apiPurchase.post(
        `/telecom/algar/products/${id}/conditions`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  async uploadProductDetails(
    ...
    try {
      const response = await apiPurchase.post(
        `/telecom/algar/products/${id}/details`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  async uploadProductExtras(
    ...
    try {
      const response = await apiPurchase.post(
        `/telecom/algar/products/${id}/extras-images`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  async updateProduct(
    ...
    const response = await apiPurchase.put(
      `/telecom/algar/products/${id}`,
      data,
    );
    return response.data;
  }
  async removeProduct(id: number) {
    await apiPurchase.delete(`/telecom/algar/products/${id}`);
  }
}
