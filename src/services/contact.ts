...
export class ContactService {
  async allContacts({
    ...
    const res = await apiPurchase.get(`/telecom/algar/messages`, {
      params: {
        page,
        data_to,
        data_from,
  ...
  async changeContactStatus({
    ...
    return apiPurchase.patch(`/telecom/algar/messages/${id}/status`, {
      status: status,
    });
  }

  async removeContact(id: number) {
    await apiPurchase.delete(`/telecom/algar/messages/${id}`);
  }
}
